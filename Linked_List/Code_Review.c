...
void recursiveReverse(ListNode **ptrHead)
{
	...
	ListNode *first = *ptrHead;		// first는 현재 head를 가리킴
	ListNode *others = first->next;	// others는 뒤집을 대상 리스트

	recursiveReverse(&others); 		// first, others가 하나씩 오른쪽으로 이동

    ListNode *cur = others;
    while (cur->next != NULL)
        cur = cur->next;

	cur->next = first;	// others 리스트 마지막 노드의 next가 first를 가리키도록 연결
	first->next = NULL;		// 원래 방향 연결 끊음
	*ptrHead = others;		// head 바꾸기
}
//////////////////////////////////////////////////////////////////////////////////
